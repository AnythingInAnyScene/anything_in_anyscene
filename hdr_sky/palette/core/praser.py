...
def init_obj(opt, logger, *args, default_file_name='default file', given_module=None, init_type='Network', **modify_kwargs):
    ...
    try:
        if given_module is not None:
            ...
            file_name = "anything_in_anyscene.hdr_sky.palette." + file_name
...
def parse(args):
    ...
    ''' set log directory '''
    experiments_root = os.path.join(opt["datasets"]["test"]["which_dataset"]["args"]["data_root"], '{}_{}'.format(opt['name'], get_timestamp()))
