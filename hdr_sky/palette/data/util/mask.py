...
def bboxes2mask(img_shape, bboxes, dtype='uint8'):
    """CZ added
    """
    bbox1, bbox2 = bboxes[0], bboxes[1]
    height, width = img_shape[:2]

    mask = np.zeros((height, width, 1), dtype=dtype)
    # mask[bbox[0]:bbox[0] + bbox[2], bbox[1]:bbox[1] + bbox[3], :] = 1
    mask[bbox1[0]:bbox1[0] + bbox1[2], bbox1[1]:bbox1[1] + bbox1[3], :] = 1
    mask[bbox2[0]:bbox2[0] + bbox2[2], bbox2[1]:bbox2[1] + bbox2[3], :] = 1

    return mask

def randombox2mask(img_shape, dtype='uint8', denumerator=2):
    """CZ added
    """
    
    height, width = img_shape[:2]
    h, w = height//denumerator, width//denumerator
    mask = np.ones((height, width, 1), dtype=dtype)

    h_offset = np.random.randint(0, height-h-1-50)
    w_offset = np.random.randint(0, width-w-1)
    mask[h_offset:h_offset+h, w_offset:w_offset+w, :] = 0

    return mask

def fixbox2mask(img_shape, dtype='uint8', denumerator=3):
    """CZ added
    """
